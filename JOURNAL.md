...
I'll be writing the app in Python, as speed of dev implementation is far more important than runtime efficiency of something like C++. Easier to get a Python environment set up on my personal laptop as well, and it's a language I'm very
familiar with.
...
I will need to explain my code at a follow up interview, so using genAI for complex aspects of the project is a bad idea. I've also heard that free AI agents very quickly run out of tokens on complex coding tasks, so I may not be able to rely on it much at all. I will try to complete this project with as few prompts as possible.

## 6:40pm - Basic Environment Up

Got everything set up for a basic dev environment and made an account for free Claude. (True Claude Code requires a paid account.) I'll try using the default Sonnet 5 Medium setting.
...
Ah, already discovered another issue. Claude organized the labels in a QGridLayout. I probably would have used something more flexible - I prefer nested V/HBoxLayouts.
...
Asked Claude to add persistent data storage via loading/saving a JSON file (another format I'm very familiar with, and can edit by hand if needed), plus click-to-edit on the status field. Of course, since I wasn't able to have it run on my modified version of the code, I'll need to hand integrate its changes.
